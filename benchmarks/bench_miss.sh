#!/usr/bin/env bash
# bench_miss.sh - Cache MISS sweep
...
RESULTSDIR="results"
mkdir -p "$RESULTSDIR"
OUTFILE="$RESULTSDIR/miss_$(date +%Y%m%d_%H%M%S).txt"
...
echo "=== BENCH mode=miss started $(date) ===" | tee "$OUTFILE"
...
for c in "${CONNECTIONS[@]}"; do
    ...
    for run in $(seq 1 "$REPEATS"); do
        echo ">>> mode=miss connections=$c threads=$t run=$run" | tee -a "$OUTFILE"
        # No cache warm-up for miss — every request must be a cache miss
        BENCH_MODE=miss taskset -c 6-9 wrk -t"$t" -c"$c" -d"$DURATION" --latency \
            -s "$SCRIPT_DIR/bench.lua" "$PROXY" | tee -a "$OUTFILE"
        sleep 2   # let TIME_WAIT sockets drain between runs
    done
done

echo "=== BENCH mode=miss finished $(date) ===" | tee -a "$OUTFILE"
